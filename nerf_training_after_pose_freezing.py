...
def train():
    ...
    optimizer_pose = optimizer1([dict(params=pose_net.parameters(), lr=3e-3)])
    ...
    img_i = -1
    pose_net.eval()
    choices = [i for i in range(len(i_train))]
    for i in trange(start, N_iters + poses.shape[0]*100*2 + 1):
        torch.cuda.empty_cache()

        if i >= args.precrop_iters_time:
            img_i = np.random.choice(choices)
        else:
            skip_factor = i / float(args.precrop_iters_time) * len(choices)
            max_sample = max(int(skip_factor), 3)
            img_i = np.random.choice(choices[:max_sample])
        ...
        refinement_round = (i-N_iters + poses.shape[0]*100*2 + 1) // args.depth_refine_period
        if not args.no_depth_refine and depth_maps is not None and (i-N_iters + poses.shape[0]*100*2 + 1) % args.depth_refine_period == 0 and refinement_round <= args.depth_refine_rounds:
        ...
        if i % 1000 == 0:
            ...
            writer.add_scalar('only_photo_loss', img_loss.item(), i)
            writer.add_scalar('only_psnr', psnr.item(), i)
            if 'rgb0' in extras:
                writer.add_scalar('only_loss0', img_loss0.item(), i)
                writer.add_scalar('only_psnr0', psnr0.item(), i)
            if args.add_tv_loss:
                writer.add_scalar('only_tv', tv_loss.item(), i)
            if depth_maps is not None:
                writer.add_scalar('olny_depth', depth_loss.item(), i)
