...
def train():
    ...
    ## Create pose optimisation network
    ...
    ## Create flow network for flow loss
    flow_net = FlowSelectionWrapper(ckpt_path='pretrained_flownets/PDCNet_megadepth.pth.tar',
                                             backbone='PDCNet',
                                             batch_size=5,
                                             num_views=poses.shape[0]).to('cuda')
    # print("flow network created :: ", flow_net)

    flowCorrespondenceLoss = CorrespondenceBasedLoss(H, W, focal, flow_net, images, poses, times, masks, render_kwargs_train['network_fn'], 'cuda')
    ...
    for i in trange(start, N_iters):
        ...
        if N_rand is not None:
            ...
            all_rays_o, all_rays_d = rays_o, rays_d
            # print("ray_o", rays_o)
            # print("rays_d", rays_d)
        ...
        rgb, disp, acc, extras = render(progress, H, W, focal, chunk=args.chunk, rays=batch_rays, frame_time=frame_time,
        ...
        flow_loss = flowCorrespondenceLoss.calculate_loss(all_rays_o, all_rays_d, coords, render_kwargs_train,int(i/poses.shape[0]), poses, progress)
        
        # print("flow loss ? : ", i,poses.shape, i/poses.shape[0]>15)
        if i/poses.shape[0]>15:
            # print("are we calculating flow loss")
            flow_loss.backward(retain_graph=True)
        
        if i%poses.shape[0] == 0:
            ##calculate the flow loss here -- 

            if i/poses.shape[0]>15:
                writer.add_scalar('simul_flow_loss', flow_loss.item(), i)
