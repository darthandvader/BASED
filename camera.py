...
class Pose():
    ...
    def invert(self,pose: torch.Tensor,use_inverse=False) -> torch.Tensor:
        ...
        R,t = pose[...,:3],pose[...,3:]
        R_inv = R.inverse() if use_inverse else R.transpose(-1,-2)
        R_inv = R_inv.type(torch.float64)
    ...
    def compose(self,pose_list: List[torch.Tensor]) -> torch.Tensor:
        ...
        ## Shreya check this if things go south
...
def get_pixels_from_camera_center_and_rays(intr: torch.Tensor, center: torch.Tensor, rays: torch.Tensor, pose_w2c: torch.Tensor, depth: torch.Tensor):

    points_3D = center+rays*depth
    R,t = pose_w2c[:3, :3],pose_w2c[:3, 3:]
    t = torch.unsqueeze(t, dim=0) 
    t = torch.squeeze(t, 2)
    t = t.expand(points_3D.shape[0], 3)
    points_3D = R@points_3D.transpose(-1,-2)
    points_3D = points_3D.transpose(0,1) + t
    pixels = intr@points_3D.transpose(-1,-2)
