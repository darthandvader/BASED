...
from two_columns import FirstTwoColunmnsPoseParameters
...
class Embedder:
    ...
    def embed(self, inputs, progress=1.0):
        # L = self.N_freqs
        # start = self.c2f[0]
        # end = self.c2f[1]
        ...
        #     weight1.append(w)
        #     weight1.append(w)
        # pe = [self.embed_fns[0](inputs)]
        # for w,fn in zip(weight1, self.embed_fns[1:]):
        #     pe.append(fn(inputs)*w)
...
def get_embedder(multires, input_dims, i=0):
    ...
    embed = lambda x, progress, eo=embedder_obj : eo.embed(x, progress)
...
class DirectTemporalNeRF(nn.Module):
    def __init__(self, D=8, W=256, input_ch=3, input_ch_views=3, input_ch_time=1, output_ch=4, skips=[4],
        ...
        # print("time out:: ", self._time_out)
        # print("nerf net :: ", self._occ)
    ...
    def forward_deformation(self, x, ts, progress):
        ts = self.embedtime_fn(ts, progress)
        input_pts = self.embed_fn(x, progress)
        cur_time = ts[0, 0]
    ...
    def forward(self, x, ts, progress):
        ...
        if cur_time == 0. and self.zero_canonical:
            ...
            input_pts = self.embed_fn(input_pts_orig + dx, progress)
...
class RecurrentTemporalNeRF(nn.Module):
    def __init__(self, D=8, W=256, input_ch=3, input_ch_views=3, input_ch_time=1, output_ch=4, skips=[4],
                 use_viewdirs=False, memory=[], embed_fn=None, embedtime_fn=None,
                 zero_canonical=True, time_window_size=1, time_interval=0.0):
        super(RecurrentTemporalNeRF, self).__init__()
        self.D = D
        self.W = W
        self.input_ch = input_ch
        self.input_ch_views = input_ch_views
    ...
    def create_time_net(self):
        ...
        out = nn.Sequential(nn.Linear(self.W, self.W // 2), nn.Linear(self.W // 2, 3))
        ...
        if cur_time == 0. and self.zero_canonical:
            ...
            for i in range(1, self.time_window_size):
                time_hidden_window.append(
                    self.query_time_hidden(
                        input_pts,
                        self.embedtime_fn(torch.maximum(torch.zeros_like(t[:, :1]), t[:, :1] - i * self.time_interval)),
                        self._time_hidden
                    )
                )
            
            time_hidden = torch.stack(time_hidden_window)
            curr_time_hidden = self.query_time_hidden(input_pts, t, self._time_hidden).unsqueeze(0)
...
class NeRFOriginal(nn.Module):
    def __init__(self, D=8, W=256, input_ch=3, input_ch_views=3, input_ch_time=1, output_ch=4, skips=[4],
                 use_viewdirs=False, memory=[], embed_fn=None, embedtime_fn=None,
                 output_color_ch=3, zero_canonical=True, time_window_size=1, time_interval=0.0):
...
# Ray helpers
def get_rays_batched(H, W, focal, c2w):
    i, j = torch.meshgrid(torch.linspace(0, W-1, W), torch.linspace(0, H-1, H))  # pytorch's meshgrid has indexing='ij'
    i = i.t()
    j = j.t()
    dirs = torch.stack([(i-W*.5)/focal, -(j-H*.5)/focal, -torch.ones_like(i)], -1)
    dirs = dirs.expand(c2w.shape[0], dirs.shape[0], dirs.shape[1],dirs.shape[2])
    # Rotate ray directions from camera frame to the world frame
    a = dirs[..., np.newaxis, :]
    b = c2w[...,:3,:3]
    ...
    rays_o = rays_o.expand(c2w.shape[0],rays_d.shape[1],rays_d.shape[2],rays_d.shape[3])
    return rays_o, rays_d
...
def ndc_rays(H, W, focal, near, rays_o, rays_d):
    ...
    # Projection
    o0 = -1./(W/(2.*focal)) * rays_o[...,0] / (rays_o[...,2] + 1e-6)
    o1 = -1./(H/(2.*focal)) * rays_o[...,1] / (rays_o[...,2] + 1e-6)
    o2 = 1. + 2. * near / (rays_o[...,2] + 1e-6)

    d0 = -1./(W/(2.*focal)) * (rays_d[...,0]/(rays_d[...,2] + 1e-6) - rays_o[...,0]/(rays_o[...,2] + 1e-6))
    d1 = -1./(H/(2.*focal)) * (rays_d[...,1]/(rays_d[...,2] + 1e-6) - rays_o[...,1]/(rays_o[...,2] + 1e-6))
    d2 = -2. * near / (rays_o[...,2] + 1e-6)
...
def importance_sampling_coords(weights, N_samples, det=False, pytest=False):
    # Get pdf
    # print("weights before", weights.shape)
