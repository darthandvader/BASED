...
def get_caninical_3d_pts(kpi, di, Ki, ti, nerf, T, progress):

    if len(di.shape) == len(kpi.shape):
        # di must be BxNx1
        di = di.squeeze(-1)
    kpi_3d_i = to_homogeneous(kpi.float()) ## pixels in homogeneous format
    kpi_3d_i = kpi_3d_i @ torch.inverse(Ki.float()).transpose(-1, -2)
    kpi_3d_i = kpi_3d_i * di[..., None]  
    kpi_3d_i = from_homogeneous(to_homogeneous(kpi_3d_i) @ T.transpose(-1, -2)) ## 3d point in world frame

    ts = ti * torch.ones_like(kpi_3d_i[...,:1])
    dx = nerf.forward_deformation(kpi_3d_i,ts, progress)
    return torch.add(kpi_3d_i,dx)

def convert_3d_points_to_pixels(k_3d, K):
    pixel = from_homogeneous(k_3d @ K.transpose(-1, -2))
    depth = None
    return None

def batch_project_to_other_img(kpi: torch.Tensor, di: torch.Tensor, Ki: torch.Tensor, Kj: torch.Tensor, 
                               T_itoj: torch.Tensor,
    ...
    if len(di.shape) == len(kpi.shape):
        # di must be BxNx1
        di = di.squeeze(-1)
    kpi_3d_i = to_homogeneous(kpi.float()) @ torch.inverse(Ki.float()).transpose(-1, -2)
    kpi_3d_i = kpi_3d_i * di[..., None]  # non-homogeneous coordinates

    ## adding time deformation part here
    # dx_t_self = net.nerf.forward_deformation(opt, kpi_3d_i, ti, net.embedder_pts, net.embedder_time)
    # kpi_3d_0 = torch.subtract(kpi_3d_i, dx_t_self)
    # dx_t_other = net.nerf.forward_deformation(opt, kpi_3d_0, tj, net.embedder_pts, net.embedder_time)
    # kpi_3d_i = kpi_3d_tj = torch.add(kpi_3d_0,dx_t_other)
    kpi_3d_j = from_homogeneous(
            to_homogeneous(kpi_3d_i) @ T_itoj.transpose(-1, -2))
    kpi_j = from_homogeneous(kpi_3d_j @ Kj.transpose(-1, -2))
    if return_depth:
